...
import { 
  ...
  Database, Cpu, Activity, Lock, Box, Sparkles 
} from 'lucide-react';
                ...
                return (
                  <div {...(!ready && { 'aria-hidden': true, 'style': { opacity: 0, pointerEvents: 'none' } })}>
                    {(() => {
                      if (!connected) {
                        return (
                          <Button onClick={openConnectModal} size="sm" className="bg-white text-black hover:bg-slate-200 font-semibold rounded-full px-6">
                            Connect Wallet
                          </Button>
                        );
                      }
                      ...
                      return (
                        <Button onClick={openAccountModal} size="sm" variant="outline" className="border-white/10 bg-white/5 hover:bg-white/10 text-white font-mono rounded-full">
                          {account.displayName}
                        </Button>
                      );
                    })()}
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-32 pb-32">
        ...
        <div className="flex flex-col items-start max-w-5xl mb-32">
          ...
          <motion.div 
            ...
            className="flex items-center gap-2 px-4 py-1.5 mb-8 rounded-full border border-blue-500/20 bg-blue-500/10 backdrop-blur-md"
          >
            ...
            <h1 className="text-6xl md:text-8xl font-bold tracking-tighter text-white leading-[1.1] flex flex-wrap gap-x-4">
              {titleWords.map((word, index) => (
                <motion.span 
                  key={index} 
                  variants={textReveal}
                  className={word === "Ecosystem." ? "text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400" : ""}
                >
                  {word}
                </motion.span>
              ))}
          ...
          >
            Experience the future of Web3 asset management. 
            A full-stack experiment integrating <span className="text-white font-medium">ERC-721 issuance</span>, 
            <span className="text-white font-medium"> dynamic staking yields</span>, and on-chain governance.
          </motion.p>
          ...
          <div className="flex flex-col gap-8 w-full sm:w-auto">
            ...
            >
              <Link href="/dashboard">
                <Button 
                  size="lg" 
                  className="w-full sm:w-auto bg-white text-black hover:bg-slate-200 px-8 h-14 text-base font-bold rounded-full transition-all hover:scale-105"
                >
              ...
              <Link href="https://github.com/miaojiayi123/nft-web" target="_blank">
                <Button 
                  size="lg" 
                  variant="outline" 
                  className="w-full sm:w-auto border-white/20 bg-white/5 hover:bg-white/10 text-white px-8 h-14 text-base rounded-full backdrop-blur-sm transition-all hover:scale-105"
                >
        ...
        <motion.div {...fadeInUp} className="mb-32">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <TechCard 
              ...
              desc="Receive 100 $KIKI testnet tokens instantly to jumpstart your ecosystem journey."
            />
            <TechCard 
              index="02"
              title="Treasury Mint"
              subtitle="Burn Mechanism"
              desc="Deflationary model. Burn 20 $KIKI to mint unique generative NFT assets."
            />
            <TechCard 
              index="03"
              title="Staking Yield"
              subtitle="Passive Income"
              desc="Stake assets to earn 0.01 $KIKI/s APY. Real-time off-chain calculation."
            />
            <TechCard 
              index="04"
              title="Token Gated"
              subtitle="Access Control"
              desc="Exclusive verification system unlocking private community channels."
            />
          </div>
        </motion.div>
